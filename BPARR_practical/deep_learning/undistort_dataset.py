...
import cv2
import json
from BPARR_practical.algorithmic.algorithmic import load_camera_intrinsic



if __name__=="__main__":
    source = Path("../resources/jetbot_datasets/dataset_9")
    destination = Path("./datasets/dataset_9_undist")
    destination.mkdir(parents=True, exist_ok=True)
    images = glob.glob(f"{source.__str__()}/*.png")

    # load undistortion parameters
    params = load_camera_intrinsic("../algorithmic/camera_intrinsic_calibration.json")
